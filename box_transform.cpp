...
"void main()\n"
"{\n"
"color = mix(texture(ourTexture1, TexCoord), texture(ourTexture2, TexCoord), 1.0);\n"
"}\n\0";
...
int main()
{
	...
	//开启深度测试：将它的深度值和z缓冲进行比较然后如果当前的片段在其它片段之后它将会被丢弃，然后重写
...
//	glGenBuffers(1, &EBO);
	...
	Mat src2 = imread("QQ图片20160808194503.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		////模型矩阵：将其绕着x轴旋转使它看起来像放在地上一样
		//model = glm::rotate(model, -45.0f, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, (GLfloat)glfwGetTime() * 1.0f, glm::vec3(0.5f, 1.0f, 1.0f));

		//观察矩阵：想要相机往后移动实际上就是把整个场景往前移动，
		//OpenGL是一个右手坐标系，将场景沿着z轴正方向平移会给我们一种我们在往后移动的感觉。
		...
		//	model = glm::rotate(model, (GLfloat)glfwGetTime()*1.0f, cubePositions[i]);
